...
- `DataManager` 是唯一 autoload，负责加载并索引主要 JSON 配置。
...
| --- | --- | --- | --- | --- | --- | --- |
| 数据配置 | `DataManager` autoload 加载索引，`GameData` 提供静态读取和 fallback | `DataManager` 与 `GameData` 双入口并存，新字段容易只接一边 | 新配置先登记到 `DataPaths`，再同步 `DataManager`、`GameData` 和验证脚本 | 需要小步收口 | 高 | 中 |
