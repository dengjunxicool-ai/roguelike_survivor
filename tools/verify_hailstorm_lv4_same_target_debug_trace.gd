...
class TestEnemy:
	...
	var enemy_id: StringName = &"debug_hail_lv4_target"
...
func _run() -> void:
	...
	skill_instance.set("runtime_special_rules", {
	...
	_expect(spawned, "hailstorm Lv4 debug spawn creates projectiles")
	var projectiles: Array[Node] = _hail_projectiles()
	_expect(projectiles.size() == 3, "hailstorm Lv4 debug spawn creates three projectiles")
	for projectile: Node in projectiles:
		projectile.call("_emit_hit_event", enemy)
	_expect(enemy.damage_amounts == [11, 9, 6], "hailstorm Lv4 debug hit damage sequence is 11/9/6")
	var damage_records: Array[Dictionary] = _damage_records(DebugCombatTraceScript.get_records(root))
	_expect(damage_records.size() == 3, "hailstorm Lv4 writes three debug damage records")
	if damage_records.size() >= 3:
		_expect(int(damage_records[0].get("final_amount", 0)) == 11, "hailstorm Lv4 first debug record remains full damage")
		_expect(int(damage_records[1].get("final_amount", 0)) == 9, "hailstorm Lv4 second debug record uses 80 percent")
		_expect(int(damage_records[2].get("final_amount", 0)) == 6, "hailstorm Lv4 third debug record uses 60 percent")
...
func _damage_records(records: Array) -> Array[Dictionary]:
...
func _write_result() -> void:
	var output: String = "\n".join(_lines)
	print(output)
	var file: FileAccess = FileAccess.open("res://tools/verify_hailstorm_lv4_same_target_debug_trace.out.txt", FileAccess.WRITE)
	if file != null:
