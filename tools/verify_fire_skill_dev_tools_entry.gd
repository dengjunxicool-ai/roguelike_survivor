...
func _run() -> void:
	...
	_expect(int(result.get("damage_record_count", 0)) >= 1, "fire skill writes damage records", result.get("damage_record_count", 0))
	_expect(int(result.get("particle_count", 0)) >= 1, "fire skill creates particles/effects", result.get("particle_count", 0))
	_expect(int(result.get("damage_popup_count", 0)) >= 1, "fire skill creates damage popup feedback", result.get("damage_popup_count", 0))
	await _teardown()
	if not _failed:
