...
func _run() -> void:
	...
	var holy_attack: RefCounted = _skill_manager.call("get_skill", &"holy_attack_judgment") as RefCounted
	_emit(&"attack_hit", holy_attack)
	_expect(_enemy.call("get_status_stack", &"judgment") > 0, "holy attack_hit applies Judgment", _enemy.call("get_status_stack", &"judgment"))

	_judged_enemy.call("apply_status", &"judgment", {"stacks": 1, "duration": 6.0, "power": 24.0})
	await create_timer(0.25).timeout
	holy_attack.set_meta("trigger_cd_attack_hit_global", 0.0)
	_player.set_meta("fire_passive_shield", 0)
	var shield_before_attack: int = int(_player.get_meta("fire_passive_shield", 0))
	_emit_to(&"attack_hit", holy_attack, _judged_enemy)
	await process_frame
	_expect(int(_player.get_meta("fire_passive_shield", 0)) > shield_before_attack, "holy attack grants shield when hitting Judgment targets", _player.get_meta("fire_passive_shield", 0))

	var dash_skill: RefCounted = _skill_manager.call("get_skill", &"holy_dash_heavenly_wings") as RefCounted
	var shield_before_dash: int = int(_player.get_meta("fire_passive_shield", 0))
	_emit_without_target(&"dash_start", dash_skill)
	await process_frame
	_expect(int(_player.get_meta("fire_passive_shield", 0)) > shield_before_dash, "heavenly wings grants dash shield", _player.get_meta("fire_passive_shield", 0))
	_expect(_count_area_effects(&"heavenly_wings_path") > 0, "heavenly wings creates a visible dash path", _count_area_effects(&"heavenly_wings_path"))

	_judged_enemy.call("apply_status", &"judgment", {"stacks": 4, "duration": 6.0, "power": 24.0})
	_emit_without_target(&"on_cast", _skill_manager.call("get_skill", &"holy_cast_holy_ray") as RefCounted)
	await process_frame
	_expect(_count_projectiles(&"holy_ray_beam") >= 3, "holy ray drops 3 visible beams", _count_projectiles(&"holy_ray_beam"))
	_expect(_projectile_damage_for(&"holy_ray_beam") > 0, "holy ray beams carry nonzero damage", _projectile_damage_for(&"holy_ray_beam"))

	_emit_without_target(&"on_cast", _skill_manager.call("get_skill", &"holy_cast_divine_barrier") as RefCounted)
	await process_frame
	_expect(_count_area_effects(&"divine_barrier_field") > 0, "divine barrier creates a visible field", _count_area_effects(&"divine_barrier_field"))

	_emit_without_target(&"on_cast", _skill_manager.call("get_skill", &"holy_cast_judgment_hammer") as RefCounted)
	await process_frame
	_expect(_count_area_effects(&"judgment_hammer_area") > 0, "judgment hammer creates a visible strike", _count_area_effects(&"judgment_hammer_area"))
	_expect(_area_position_near(&"judgment_hammer_area", _elite_enemy.global_position, 8.0), "judgment hammer targets highest HP enemy without explicit target", _area_position_for(&"judgment_hammer_area"))

	_emit(&"on_cast", _skill_manager.call("get_skill", &"holy_summon_seraph") as RefCounted)
	_emit(&"on_cast", _skill_manager.call("get_skill", &"holy_summon_shield_guardian") as RefCounted)
	await process_frame
	...
	if not _failed:
		print("[verify_holy_skill_runtime_smoke] PASS")
	quit(1 if _failed else 0)


func _build_nodes() -> void:
	_player = SmokePlayer.new()
	_player.name = "HolySmokePlayer"
	...
	_enemy = _create_enemy("HolySmokeEnemy", Vector2(140.0, 0.0))
	_judged_enemy = _create_enemy("JudgedHolySmokeEnemy", Vector2(180.0, 22.0))
	_elite_enemy = _create_enemy("EliteHolySmokeEnemy", Vector2(260.0, 0.0))
...
func _load_holy_skill_ids() -> Array[StringName]:
	...
	for skill_variant: Variant in (parsed as Dictionary).get("skills", []):
		...
		if str(skill.get("school", "")) != "holy" or skill.get("fusion_school", null) != null:
			continue
...
func _emit_to(event_name: StringName, skill_instance: RefCounted, target: Node2D) -> void:
	var context: Dictionary = _base_context(skill_instance)
	context["target"] = target
	context["enemy"] = target
...
func _base_context(skill_instance: RefCounted) -> Dictionary:
	return {
		...
		"damage_packet": {
			...
			"element": &"holy",
			"source_skill_id": &"holy_attack_judgment"
		}
	}
...
func _fail(label: String, actual: Variant = "") -> void:
	_failed = true
	push_error("[verify_holy_skill_runtime_smoke] FAIL %s actual=%s" % [label, str(actual)])
